# Final test script with unique data to avoid all conflicts
Write-Host "=== FINAL API TESTING WITH UNIQUE DATA ===" -ForegroundColor Cyan
Write-Host ""
...
$adminEndpoints = @(
    @{ Method = "POST"; Path = "/admin/movies"; Name = "Add Movie (Admin)"; Body = @{
        title = "Final Test Movie $timestamp"
        description = "A test movie for final API testing"
        duration_mins = 120
        genre = @("Action", "Drama")
        language = @("English")
        format = @("2D", "3D", "IMAX")
...
Write-Host "`n=== FINAL TESTING COMPLETE ===" -ForegroundColor Cyan
Write-Host "All APIs have been tested with unique data!" -ForegroundColor Green 